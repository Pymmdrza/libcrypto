...
#
# where mode_state is a pointer to base_cipher_state plus mode-specific data.
...
def _create_cipher(factory, key, mode, *args, **kwargs):
    ...
    if mode == 1:
        from ..Cipher._mode_ecb import _create_ecb_cipher
        res = _create_ecb_cipher(factory, **kwargs)
    elif mode == 2:
        from ..Cipher._mode_cbc import _create_cbc_cipher
        res = _create_cbc_cipher(factory, **kwargs)
    elif mode == 3:
        from ..Cipher._mode_cfb import _create_cfb_cipher
        res = _create_cfb_cipher(factory, **kwargs)
    elif mode == 5:
        from ..Cipher._mode_ofb import _create_ofb_cipher
        res = _create_ofb_cipher(factory, **kwargs)
    elif mode == 6:
        from ..Cipher._mode_ctr import _create_ctr_cipher
        res = _create_ctr_cipher(factory, **kwargs)
    elif mode == 7:
        from ..Cipher._mode_openpgp import _create_openpgp_cipher
        res = _create_openpgp_cipher(factory, **kwargs)
    elif mode == 9:
        from ..Cipher._mode_eax import _create_eax_cipher
        res = _create_eax_cipher(factory, **kwargs)
    elif extra_modes:
        if mode == 8:
            from ..Cipher._mode_ccm import _create_ccm_cipher
            res = _create_ccm_cipher(factory, **kwargs)
        elif mode == 10:
            from ..Cipher._mode_siv import _create_siv_cipher
            res = _create_siv_cipher(factory, **kwargs)
        elif mode == 11:
            from ..Cipher._mode_gcm import _create_gcm_cipher
            res = _create_gcm_cipher(factory, **kwargs)
        elif mode == 12:
            from ..Cipher._mode_ocb import _create_ocb_cipher
            res = _create_ocb_cipher(factory, **kwargs)
        elif mode == 13:
            from ..Cipher._mode_kw import _create_kw_cipher
            res = _create_kw_cipher(factory, **kwargs)
        elif mode == 14:
            from ..Cipher._mode_kwp import _create_kwp_cipher
